...
def init_logger(
  ...
  Args:
      log_dir (Union[str, Path], optional): 日志保存目录. 默认从配置文件读取.
      rotation (str, optional): 日志轮转条件（大小或时间）. 默认从配置文件读取.
      retention (str, optional): 日志保留时长. 默认从配置文件读取.
      console_level (str, optional): 控制台输出级别. 默认从配置文件读取.
      file_level (str, optional): 文件输出级别. 默认从配置文件读取.
  """
  # 从配置文件加载日志配置
  log_dir = log_dir or config.get("logger.path", "logs")
  rotation = rotation or config.get("logger.rotation", "10 MB")
  retention = retention or config.get("logger.retention", "7 days")
  log_level = config.get("logger.level", "DEBUG")
  console_enabled = config.get("logger.console", True)
